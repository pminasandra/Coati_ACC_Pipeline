...
def acc_plots_all_audits():
    ...
    audit_count = 1
    for auditname in all_audits:
        df_audit = all_audits[auditname]
        ...
        axs[0].legend()
        axs[0].set_title(os.path.basename(auditname))
        axs[0].set_xticks(rotation=90)
        utilities.saveimg(fig, f"audit{audit_count}")
        audit_count += 1
        plt.close(fig)

#def feature_plots_one_audit():

if __name__ == "__main__":
    acc_plots_all_audits()
